...
public class TC001_CreateOrganization_By_Excel {
...
public Object[][] getOrganizationData() throws Throwable{
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			objArr[i][j]= excelLib.getDataFromExcel("Organization", i, j);
			objArr[i][j]= excelLib.getDataFromExcel("Organization", i, j);
			objArr[i][j]= excelLib.getDataFromExcelUsingDataFormatter("Organization", i, j);
		}
	}
	return objArr;
}
